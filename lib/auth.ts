...
export async function hashPassword(password:string,salt=hex(bytes(16))){
 const key=await crypto.subtle.importKey('raw',new TextEncoder().encode(password),'PBKDF2',false,['deriveBits']);
 return `pbkdf2$100000$${salt}$${hex(await crypto.subtle.deriveBits({name:'PBKDF2',hash:'SHA-256',salt:new TextEncoder().encode(salt),iterations:100000},key,256))}`;
}
export async function verifyPassword(password:string,hash:string){
 const actual=await hashPassword(password,hash.split('$')[2]);let diff=actual.length^hash.length;
